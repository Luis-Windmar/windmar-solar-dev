...
export default function WelcomeScreen() {
  ...
  const [screen, setScreen]       = useState("welcome"); // welcome | exit | upload | roof | serviceType | estimate | contact | thankyou-yes | thankyou-no | offgrid | offgridThankyou
  ...
  const handleContinue = () => {
    if (!selection) return;
    if (selection === "si")      setScreen("upload");
    else if (selection === "offgrid") setScreen("offgrid");
    else                         setScreen("exit");
  };
  ...
  if (screen === "offgrid") {
    return (
      <OffGridScreen
        ...
        onDone={() => setScreen("offgridThankyou")}
      />
    );
  }
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.content}>
        ...
        {/* DEMO TOGGLE — visible only in TEST_MODE. In production the
            generateLead state stays at its useState(true) default and Zoho
            always fires. */}
        {TEST_MODE && (
