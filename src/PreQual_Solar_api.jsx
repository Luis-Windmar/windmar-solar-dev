...
export default function PreQualSolar() {
  ...
  const process = (txt) => {
    ...
    if (step === 'PHONE') {
      ...
      if (data.phone_pending !== undefined) {
        ...
        // Build full session payload for handoff
        const sessionPayload = {
          nombre:             data.nombre,
          phone:              data.phone_pending,
        ...
        // Save lead + encrypt session data in parallel
        Promise.all([
          fetch('/api/leads', {
            method: 'POST',
        ...
        .then(([leadRes, encRes]) => {
          ...
          const url    = `http://localhost:3001/deal?d=${encodeURIComponent(token)}&leadId=${encodeURIComponent(leadId)}`;
          say('__LINK__','DONE',{type:'link_message', href: url, label:'📋 Llenar cuestionario detallado'});
        })
        .catch(err => {
          console.error('Handoff error:', err);
          // Fallback: show link without encryption if server call fails
          say('__LINK__','DONE',{type:'link_message', href:'http://localhost:3001/deal', label:'📋 Llenar cuestionario detallado'});
        });
          setStep('DONE'); setLoading(false); return;
        } else {
  ...
  return (
    <div className="flex flex-col h-screen" style={{background:"#EBF1FF"}}>
      <div className="p-3 shadow-lg flex items-center gap-3">
        <img src="/logo.png" alt="Windmar" style={{height:"52px",width:"auto"}} />
        <div>
          <p className="text-sm font-semibold" style={{color:"#1B3F8B"}}>Estimado Solar Comercial</p>
          <p className="text-xs" style={{color:"#F5A623"}}>para su negocio</p>
        </div>
        {testMode && (
          <span className="ml-auto text-xs rounded px-2 py-1">
            ...
            {messages.map((m,i)=>(
              <div key={i} className={`flex ${m.role==='user'?'justify-end':'justify-start'}`}>
                {m.type === 'ocr_review' ? (
                  ...
                  <div style={m.role==="user" ? {background:"#1B3F8B"} : {}} className={`max-w-[75%] rounded-2xl px-4 py-3 text-sm whitespace-pre-wrap shadow-sm ${
                    m.role==='user'
                      ? 'text-white'
